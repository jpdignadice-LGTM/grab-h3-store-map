...
try:
    ...
    if missing:
        ...
        # Initialize map directly over Manila
        ...
        for index, row in raw_df.iterrows():
            try:
                # Force strictly numeric data conversion
                lat = clean_numeric(row["LATITUDE"])
                lon = clean_numeric(row["LONGITUDE"])

                # CRITICAL: Drop any points with unmapped or 0.0 coordinates
                if lat == 0.0 or lon == 0.0 or pd.isna(lat) or pd.isna(lon):
                    continue

                # Filter out outliers outside the Philippines bounds
                if not (13.0 <= lat <= 16.0 and 119.0 <= lon <= 123.0):
                    continue

                sales_col = "Sales" if "Sales" in raw_df.columns else (
                    "Net Sales" if "Net Sales" in raw_df.columns else None)
                ...
                if h3_address not in h3_clusters:
                    h3_clusters[h3_address] = {"count": 0, "total_spd": 0.0, "total_sales": 0.0, "total_apc": 0.0,
                                               "store_details": []}
